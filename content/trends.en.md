...
<h2>6. Zaragoza is a hot and dry region</h2>
...
<h2 id="grid-co2">13. Grid-electricity CO₂: the same power, counted two ways</h2>
...
<p>A large datacentre runs day and night, so its electricity has to be generated somewhere. Even on Spain's relatively clean grid, the location-based figures above work out to hundreds of thousands of tonnes of CO₂ a year per big site — the equivalent of tens to hundreds of thousands of extra cars on the road (an average car emits roughly 2 t CO₂/yr). That adds to the warming already lengthening Aragón's heatwaves and droughts.</p>
<p>The "carbon-neutral" claim is an <em>accounting</em> result, not a physical one — and three consequences follow:</p>
<ul>
<li><strong>The emissions are still emitted.</strong> The site draws from the same wires as local homes and industry; buying renewable certificates elsewhere does not stop the electricity it actually consumes from being generated.</li>
<li><strong>It competes for the region's clean power.</strong> When a datacentre contracts a whole solar or wind farm's output, that renewable energy powers the datacentre instead of displacing fossil generation for everyone else — so Aragón's renewable build-out can be captured by hyperscalers rather than decarbonising the community.</li>
<li><strong>New demand can raise fossil output.</strong> A load this large, added quickly, is often met at the margin by gas plants — so in practice it can <em>increase</em> the grid's real emissions even while the datacentre reports zero.</li>
</ul>
<p>So the climate cost is real and shared by everyone, and the local question is who benefits from Aragón's wind and sun — the residents, or the datacentres.</p>
